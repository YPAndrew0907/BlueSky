...
run_history_backfill() {
  ...
  local history_rq1_stage="${HISTORY_RQ1_STAGE:-all}"
...
run_realtime_backfill() {
  ...
  # Realtime backfill is intentionally limited to backfill lanes only. It must
  # not rebuild or rediscover the fixed frozen panel.
  ROOT="$ROOT" \
  OUT_BASE="$OUT_BASE" \
  PYTHON_BIN="$PYTHON_BIN" \
...
run_fixed_panel() {
  # Fixed-panel mode is the frozen 1500-feed study. Keep discovery/panel-build
  # jobs off so the main collection path cannot drift.
  ROOT="$ROOT" \
  OUT_BASE="$OUT_BASE" \
  ENV_PATH="$ENV_PATH" \
  PYTHON_BIN="$PYTHON_BIN" \
  SOCKET_PATH="$SOCKET_PATH" \
  COLLECTOR_MODE="micro5" \
  DEFAULT_STUDY_ID="$DEFAULT_STUDY_ID" \
  STUDY_ID="$STUDY_ID" \
  ENABLE_INDEX_FEED_GENERATORS=0 \
  ENABLE_REFRESH_DISCOVERY=0 \
  ENABLE_BUILD_PANEL=0 \
  ENABLE_WIDE_SWEEP=0 \
